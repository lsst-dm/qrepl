...
void Response::sendStatus(unsigned int status)
{
    ...
    entStr << "<title>" << status << " " << statusStr << "</title></head>" << std::endl;
    entStr << "<body bgcolor=\"white\">" << std::endl;
...
void Response::send(std::string const& content, std::string const& contentType)
{
    ...
    boost::asio::async_write(*_socket, _responsebuf,
...
void Response::sendFile(boost::filesystem::path const& path)
{
    ...
    if (ct == contentTypesByExtension.end()) ct = contentTypesByExtension.find(".htm");
    headers["Content-Type"] = ct->second;

    headers["Content-Length"] = std::to_string(boost::filesystem::file_size(path));
    boost::filesystem::ifstream responseFile(path);
